...
to move-unhappy-turtles
  ifelse active_segregation [
     if any? turtles with [color = red and not happy?] [
         ask turtles with [ color = red and not happy? ]
     [ find-new-spot ]
     ]
     
     if all? turtles with [color = red] [happy?] [
       ask turtles with [ color = green and not happy? ]
     [ find-new-spot ]
     ]
...
to find-new-spot
  rt random-float 360
  fd random-float move-radius
  
   ifelse (active_segregation and color = red) [
      if any? other turtles-here with [color = green] [
        ask one-of turtles-here with [color = green] [ find-new-spot ]  
      ]
    if any? other turtles-here with [color = red]  [ find-new-spot ]  
   ][
...
to update-turtles
  ask turtles [
    ...
    set similar-nearby count  (turtles in-radius radius); neighbors) + c'était "turtles-on"
      with [color = [color] of myself]
    set other-nearby count (turtles in-radius radius);neighbors) + c'était "turtles-on"
      with [color != [color] of myself]
    set total-nearby similar-nearby + other-nearby

   ifelse (happy-counter >=  stay-happy-after) [
     set happy? true
  ][
    set happy? similar-nearby >= ( %-similar-wanted * total-nearby / 100 ) and similar-nearby <=  ( 1 - %-different-wanted / 100 ) * total-nearby and (total-nearby - 1) <= ( 1 - %-empty-wanted / 100 ) * count patches in-radius radius
  ]
    ifelse happy?  [
      if happy-counter < stay-happy-after [
        set happy-counter happy-counter + 1
      ]
    ][
    set happy-counter 0
    ]
...
337
462
Paramètre de ségrégation de Schelling
11
0.0
1
...
1228
450
Préférence pour une faible densité
11
0.0
1
...
1254
542
Echelle de ressenti (nombre de cases)
11
0.0
1
...
1133
276
Capacité à déménager loin du domicile
11
0.0
1
...
1192
365
Présence d'un mécanisme actif de ségrégation
11
0.0
1
