globals [
  ...
  list-connex-componants
  count-connex-componants
  number-connex-components
  finito
]
...
to setup
  set finito 0
  set number-connex-components 0
  clear-all
  if number > count patches
    [ user-message (word "This pond only has room for " count patches " turtles.")
      stop ]

  ask n-of number-forbidden patches with [(pxcor + pycor) / 2 = round ((pxcor + pycor) / 2)] [
    set forbidden 1
   
  ]
  repeat 2 [diffuse forbidden par-diffusion]
  ask patches with [forbidden > 0.2]
  [set forbidden 1
  ...
  ask n-of (count patches with [forbidden = 1] - number-forbidden) patches with [forbidden = 1] [
    set forbidden 0
   
  ]
  
  ask patches with [forbidden = 1] [
     set pcolor yellow
  ]
  
  ;; create turtles on random patches.
  ask n-of number patches with [forbidden = 0]
    [ sprout 1
      [ set color red ] ]
...
to go
   if all? turtles [happy?] or ticks = 500 [
    compute-connex-component
    set finito 1]
...
to find-new-spot
  rt random-float 360
  fd random-float 10
  if any? other turtles-here or [forbidden] of patch-here = 1
...
to compute-connex-component
 ; show "je rentre dans compute connex component"
initialize-connex-component
repeat 2 * (world-height + world-width) [iterate-connex-component]
finish-connex-component
set number-connex-components length list-connex-componants - 1

end

to test-cc
  ask patches with [forbidden = 1] [set pcolor yellow]
 let tempC 0
  let temp 0
  let temp2 0
  let temp3 0
  ask turtles [
    set tempC 0
    set temp2 color
    set temp3 [id-connex-componant] of patch-here
    set temp one-of turtles with [distance myself <= 1] 
   ;show temp
   if [color] of temp =  temp2 [
     ask temp [set tempC [id-connex-componant] of patch-here]
     ifelse temp3 < tempC [
        ask patch-here [set id-connex-componant tempC]
     ][
     ask temp[ask patch-here [set id-connex-componant temp3]]
     ]
     
    
  ]
  ] ask patches [set pcolor id-connex-componant]
  
end


to initialize-connex-component
  ;initialisation
  ask patches with [count turtles-here > 0] [set id-connex-componant (max-pxcor - pxcor) + (max-pxcor - min-pxcor + 1) * (max-pycor - pycor) + 1.5 ]
  ask patches with [forbidden = 1] [
    if any? turtles in-radius 1.1 with [color = green] [sprout 1 [set color green]]
    if any? turtles in-radius 1.1 with [color = red] [sprout 1 [set color red]]
    if count turtles-here > 1 [ask turtles-here [die]]
  ]
end

to iterate-connex-component
  let tempC 0
  let temp 0
  let temp2 0
  let temp3 0
...
to finish-connex-component
  let miccp max [id-connex-componant] of patches
  ask patches [set pcolor id-connex-componant]
  ask patches with [forbidden = 1] [
    set pcolor yellow
  ...
  set list-connex-componants []
  set count-connex-componants []
  let temp 0
  let k 0
  ask patches with [count turtles-here > 0 and [color] of one-of turtles-here = green] [
    set temp id-connex-componant
    if not member? id-connex-componant list-connex-componants [
      set list-connex-componants lput  id-connex-componant list-connex-componants
      set count-connex-componants lput 1 count-connex-componants
    ]
    if member? id-connex-componant list-connex-componants [
      set k position id-connex-componant list-connex-componants
      set count-connex-componants replace-item k count-connex-componants (item k count-connex-componants + 1)
    ]
  ]
    ask patches with [count turtles-here > 0 and [color] of one-of turtles-here = red] [
    set temp id-connex-componant + miccp
    if not member? temp list-connex-componants [
      set list-connex-componants lput  temp list-connex-componants
      set count-connex-componants lput 1 count-connex-componants
    ]
    if member? temp list-connex-componants [
      set k position temp list-connex-componants
      set count-connex-componants replace-item k count-connex-componants (item k count-connex-componants + 1)
    ]
  ]
 ; show length list-connex-componants - 1
...
1179
347
iterate-connex-componen
iterate-connex-component\nask patches [set pcolor id-connex-componant]
T
1
T
OBSERVER
...
@#$#@#$#@
<experiments>
  <experiment name="experiment" repetitions="270" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="501"/>
    <exitCondition>finito = 1</exitCondition>
    <metric>count turtles</metric>
    <metric>number-connex-components</metric>
    <metric>ticks</metric>
    <metric>percent-similar</metric>
    <metric>percent-unhappy</metric>
    <metric>count-connex-componants</metric>
    ...
    <steppedValueSet variable="par-diffusion" first="0" step="0.1" last="1"/>
    <enumeratedValueSet variable="number-forbidden">
      <value value="400"/>
    </enumeratedValueSet>
  </experiment>
</experiments>
